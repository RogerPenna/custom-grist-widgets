...
export const RelationshipLines = (() => {
  ...
  function _drawArrow(svg, startEl, endEl, opts) {
    ...
    const outlineWidth = opts.outlineSize !== undefined ? opts.outlineSize * opts.width : (opts.outlineWidth || 0);
    const outlineColor = opts.outlineColor || 'rgba(255,255,255,0.55)';
    ...
    if (opts.outline || outlineWidth > 0) {
      ...
      outline.setAttribute('stroke-width', opts.width + outlineWidth * 2);
  ...
  function _redraw(bscData, opts) {
    ...
    // Map new option names to internal names
    const finalOpts = {
      ...DEFAULT_OPTIONS,
      color: opts.color || DEFAULT_OPTIONS.color,
      width: opts.size || opts.width || DEFAULT_OPTIONS.width,
      outline: opts.outline !== undefined ? opts.outline : true,
      outlineColor: opts.outlineColor || DEFAULT_OPTIONS.outlineColor,
      outlineSize: opts.outlineSize !== undefined ? opts.outlineSize : 0.2
    };
      ...
      p.objectives.forEach(o => {
        ...
        targets.forEach(targetId => {
          if (targetId && targetId > 0) {
            // INVERSION: The targetId from the field is the ORIGIN (Cause)
            // The current objective 'o' is the DESTINATION (Effect)
            const startEl = document.getElementById(`record-${objTable}-${targetId}`);
            const endEl   = document.getElementById(`record-${objTable}-${o.id}`);
