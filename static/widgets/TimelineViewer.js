...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function initializeAndUpdate() {
        console.log(`[TimelineViewer] initializeAndUpdate. ConfigId: ${currentConfigId}`);
        ...
        try {
            ...
            if (currentFilter && currentFilter.column && currentFilter.value && !currentFilter.disableFiltering) {
                console.log(`[TimelineViewer] Aplicando filtro drill-down: ${currentFilter.column} = ${currentFilter.value}`);
    ...
    subscribe('grf-trigger-widget', async (data) => {
        if (data.configId && data.componentType?.toLowerCase().replace(/\s+/g, '') === 'timeline') {
