...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function initializeAndUpdate() {
        if (!tableLens) {
            try { tableLens = new GristTableLens(window.grist); } catch (e) { console.warn("[Gantt Widget] Grist not ready yet"); return; }
        }
        const options = await window.grist.getOptions() || {};
        const newConfigId = options.configId;

        if (!newConfigId) {
            container.innerHTML = '<div class="status-placeholder">Widget de Gráfico de Gantt não configurado. Use a engrenagem ⚙️ para vincular um ID de configuração.</div>';
            return;
        }
        ...
        try {
            ...
            console.error("[Gantt Widget] Erro:", e);
